...
!developmentChains.includes(network.name)
    ...
    : describe("Raffle Unit Tests", function () {
          ...
          describe("Perform Upkeep", function () {
              ...
              it("updates the raffleState, emits an event and call vrf Coordinator", async function () {
                  await raffle.enterRaffle({ value: raffleEntranceFee })
                  await network.provider.send("evm_increaseTime", [interval.toNumber() + 1])
                  await network.provider.send("evm_mine", [])
                  ...
                  const requestId = txReceipt.events[1].args.requestId //we put one as it will be second event one event is builting and second event is put by us in smart contract
                  const raffleState = await raffle.getRaffleState()
